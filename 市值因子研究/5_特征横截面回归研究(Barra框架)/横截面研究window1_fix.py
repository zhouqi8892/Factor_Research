...
def before_market_open(context):
    ...
    def char_df_gen(market_df):
        ...
        char_df = pd.DataFrame(columns = ['day','code']+g.char_list)
        for date in date_list:
            code_list = list(market_df.code[market_df.date == date].values)
            result = get_factor_values(securities=code_list,factors=g.char_list,
                                        start_date=date, end_date=date)
            char_date_df = pd.DataFrame()
            char_date_df['day'] = list(result[g.char_list[0]].index.strftime('%Y-%m-%d').values)*len(code_list)
            char_date_df['code'] = code_list
            for j in g.char_list:
                r = result[j]
                char_date_df[j] = r.iloc[0].values.tolist()
            char_df = char_df.append(char_date_df)
        char_df['day']=pd.to_datetime(char_df['day'])
        # normalization process
        char_df['size'] = (char_df['size']-char_df['size'].mean())/char_df['size'].std()
        return char_df
...
def on_strategy_end(context):
    '''在回测结束时调用'''
    write_file('summary_cs_normalized_lag1_df.csv', g.summary_df.to_csv(), append=False)
